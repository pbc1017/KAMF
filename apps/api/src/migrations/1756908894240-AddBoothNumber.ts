...
export class AddBoothNumber1756908894240 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`booths\` ADD \`boothNumber\` varchar(10) NULL`
    );

    // Update existing booth data to set boothNumber = id as string
    await queryRunner.query(
      `UPDATE \`booths\` SET \`boothNumber\` = CAST(\`id\` AS CHAR)`
    );

    // Make boothNumber NOT NULL and add unique constraint
    await queryRunner.query(
      `ALTER TABLE \`booths\` MODIFY \`boothNumber\` varchar(10) NOT NULL`
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`booths\` DROP INDEX \`IDX_booth_number\``
    );
